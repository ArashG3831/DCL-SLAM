...
public:
  ...
  [[nodiscard]] std::pair<double, double> mapToWorld(int mx, int my) const
  {
    ...
    const double local_x = (static_cast<double>(mx) + 0.5) * info.resolution;
    const double local_y = (static_cast<double>(my) + 0.5) * info.resolution;
    const double yaw = originYaw(info);
    const double cosine = std::cos(yaw);
    const double sine = std::sin(yaw);
    const double wx = info.origin.position.x + cosine * local_x - sine * local_y;
    const double wy = info.origin.position.y + sine * local_x + cosine * local_y;
    return {wx, wy};
  }
  ...
  [[nodiscard]] std::pair<int, int> worldToMap(double wx, double wy) const
  {
    int mx = 0;
    int my = 0;
    if (!worldToMapNoThrow(wx, wy, mx, my)) {
  ...
  [[nodiscard]] bool worldToMapNoThrow(double wx, double wy, int & mx, int & my) const noexcept
  {
    ...
    const int maybe_mx = static_cast<int>(std::floor(local_x / info.resolution));
    const int maybe_my = static_cast<int>(std::floor(local_y / info.resolution));
  ...
  [[nodiscard]] bool isGeometryAlignedWith(const OccupancyGrid2d & other) const
  {
    ...
    const auto & this_orientation = this_info.origin.orientation;
    const auto & other_orientation = other_info.origin.orientation;
    const double orientation_dot = std::abs(
      this_orientation.x * other_orientation.x +
      this_orientation.y * other_orientation.y +
      this_orientation.z * other_orientation.z +
      this_orientation.w * other_orientation.w);
    return (
      ...
      std::abs(orientation_dot - 1.0) <= epsilon);
  }

private:
  [[nodiscard]] static double originYaw(const nav_msgs::msg::MapMetaData & info) noexcept
  {
...
struct FrontierCandidate
{
  ...
  int size{0};
  // Exact map-cell membership retained for bounded diagnostic consumers.
  // Production scoring and dispatch use the scalar size and geometry fields.

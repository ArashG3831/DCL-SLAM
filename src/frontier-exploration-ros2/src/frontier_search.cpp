  ...
  FrontierSearchContext * search_context)
{
  ...
  result.cells.reserve(new_frontier.size());
  for (auto * frontier_point : new_frontier) {
    result.cells.emplace_back(frontier_point->mapX, frontier_point->mapY);
  }
  return result;
}
  ...
  const std::optional<FrontierCandidate::CellBounds> & visible_reveal_bounds)
{
  ...
  for (int ray_index = 0; ray_index < ray_count; ++ray_index) {
    ...
    bool entered_visible_reveal_bounds = !visible_reveal_bounds.has_value();
    for (double distance_m = step_distance_m; distance_m <= range_m; distance_m += step_distance_m) {
      ...
      {
        // The approach pose may be outside the bounded frontier cluster.  Let
        // the ray reach the cluster before applying the virtual bound; stopping
        // at the approach side incorrectly reports zero visible gain.
        if (entered_visible_reveal_bounds) {
          break;
        }
        continue;
      }
      entered_visible_reveal_bounds = true;

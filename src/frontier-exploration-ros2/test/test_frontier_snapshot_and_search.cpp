...
TEST(FrontierSearchTests, RotatedGridOriginWorldMapRoundTrip)
{
  auto grid = build_grid(4, 4, 0);
  ...
  const OccupancyGrid2d occupancy(grid);

  const auto world = occupancy.mapToWorld(0, 1);
  EXPECT_NEAR(world.first, -0.5, 1e-12);
  EXPECT_NEAR(world.second, 2.5, 1e-12);
  EXPECT_EQ(occupancy.worldToMap(world.first, world.second), std::make_pair(0, 1));
}

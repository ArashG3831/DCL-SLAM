...
enum class InternalState{WAITING_FOR_INPUTS,IDLE,PROPOSING,ARBITRATING,NAVIGATING,TERMINAL_BROADCAST,COOLDOWN,SELECTING_NEXT,LOCALLY_EXHAUSTED,MISSION_COMPLETE,IDLE_STOPPED};
...
bool action_callback_matches(
  uint64_t callback_generation, uint64_t callback_claim_id,
  uint64_t active_generation, uint64_t active_claim_id,
  InternalState current_state, InternalState expected_state);
std::string classify_nav2_failure(const std::string & error_message);
...
public:
 explicit PeerStatusTracker(ProtocolConfig config):config_(std::move(config)){}
 SessionDecision accept(const Status&,double now_s,bool active_session_fresh);
 bool expired(double now_s)const;
 std::optional<Status> status(double now_s)const;
 double age(double now_s)const;
 size_t retired_count()const{return retired_.size();}
 std::string active_key()const{return active_key_;}
private:
 void retire_active();
 ProtocolConfig config_;
 std::string active_key_;
 uint64_t highest_revision_{0};
 double received_at_{0},ttl_{0};
 std::optional<Status> status_;
 std::deque<std::string> retired_;
};
ValidationCode validate_status_shape(const Status&,const ProtocolConfig&);
struct RoundCore{InternalState state{InternalState::WAITING_FOR_INPUTS};uint64_t claim_id{0};uint32_t proposals{0};uint32_t goals_sent{0};uint32_t goals_accepted{0};bool budget_consumed{false};bool cancellation_requested{false};
 bool can_propose(uint32_t max,bool continuous=false)const{return (continuous||!budget_consumed)&&proposals<max;} void create_proposal();bool dispatch_once();bool accept_goal(bool continuous=false);void reset_cycle();void terminal_stop();};
}

...
ClearanceEvidence clearance_evidence(const frontier_exploration_ros2::OccupancyGrid2d&m,double wx,double wy,double clear,int threshold,double trace_radius){
  ...
  for(int y=cy-r;y<=cy+r;y++)for(int x=cx-r;x<=cx+r;x++){
    ...
    if(distance<=trace_radius)out.examined_cells.push_back(std::to_string(x)+":"+std::to_string(y)+":"+std::to_string(cost)+":"+std::to_string(distance));
...
bool clearance_ok(const frontier_exploration_ros2::OccupancyGrid2d&m,double wx,double wy,double clear,int threshold){return clearance_evidence(m,wx,wy,clear,threshold).accepted;}

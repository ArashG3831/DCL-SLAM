...
private:
 ...
 void start_cycle(nav_msgs::msg::OccupancyGrid::ConstSharedPtr map, nav_msgs::msg::OccupancyGrid::ConstSharedPtr cost,uint64_t rev,double rx,double ry,double yaw){
  ...
  for(auto&r:regions){
   double len=r.cells.size()*gm.resolution();if(len+1e-9<minimum_frontier_length_m_)continue;uint64_t id=stable_frontier_id(r,gm,stable_id_quantization_m_);if(suppressed(id,rev))continue;
   auto candidates=r.approach_cells;std::sort(candidates.begin(),candidates.end(),[&](auto a,auto b){auto wa=gm.mapToWorld(a.x,a.y),wb=gm.mapToWorld(b.x,b.y);return std::hypot(wa.first-r.centroid_x,wa.second-r.centroid_y)<std::hypot(wb.first-r.centroid_x,wb.second-r.centroid_y);});
   std::optional<frontier_exploration_ros2::Cell> chosen;double chosen_wx=0,chosen_wy=0;
   for(auto c:candidates){auto w=gm.mapToWorld(c.x,c.y);int cx,cy;if(std::hypot(w.first-rx,w.second-ry)<minimum_robot_distance_m_||!gc.worldToMap(w.first,w.second,cx,cy)||gc.cost(cx,cy)<0||gc.cost(cx,cy)>=costmap_blocked_threshold_)continue;
    auto safe=find_safe_approach(gc,w.first,w.second,r.centroid_x,r.centroid_y,.75,planner_tolerance_m_+.5*gc.resolution()+1e-6,approach_clearance_m_,costmap_blocked_threshold_);if(!safe)continue;auto sw=gc.mapToWorld(safe->x,safe->y);chosen=c;chosen_wx=sw.first;chosen_wy=sw.second;break;
   }
   if(!chosen){suppress(id,rev,true);continue;}auto w=std::pair{chosen_wx,chosen_wy};Work x{r,id,len,std::hypot(w.first-rx,w.second-ry),0};x.heading=std::abs(std::atan2(std::sin(std::atan2(r.centroid_y-w.second,r.centroid_x-w.first)-yaw),std::cos(std::atan2(r.centroid_y-w.second,r.centroid_x-w.first)-yaw)));x.pose.header.frame_id=global_frame_;x.pose.header.stamp=now();x.pose.pose.position.x=w.first;x.pose.pose.position.y=w.second;double gyaw=std::atan2(r.centroid_y-w.second,r.centroid_x-w.first);x.pose.pose.orientation.z=std::sin(gyaw/2);x.pose.pose.orientation.w=std::cos(gyaw/2);works_.push_back(std::move(x));}

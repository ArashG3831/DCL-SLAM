...
private:
 void map_cb(nav_msgs::msg::OccupancyGrid::ConstSharedPtr m){auto sum=map_checksum(*m);const auto receipt=std::chrono::steady_clock::now().time_since_epoch();std::lock_guard<std::mutex>l(mu_);const bool changed=!latest_map_||sum!=map_sum_;latest_map_=m;if(changed){map_sum_=sum;++revision_;pending_=true;}RCLCPP_INFO(get_logger(),"GENERATOR_MAP_RECEIPT topic=%s receipt_steady_ns=%ld header=%d.%09u checksum=%lu changed=%s content_revision=%lu dimensions=%ux%u origin=(%.3f,%.3f) resolution=%.3f",map_topic_.c_str(),std::chrono::duration_cast<std::chrono::nanoseconds>(receipt).count(),m->header.stamp.sec,m->header.stamp.nanosec,sum,changed?"true":"false",revision_,m->info.width,m->info.height,m->info.origin.position.x,m->info.origin.position.y,m->info.resolution);}
 void cost_cb(nav_msgs::msg::OccupancyGrid::ConstSharedPtr m){auto sum=map_checksum(*m);const auto receipt=std::chrono::steady_clock::now().time_since_epoch();std::lock_guard<std::mutex>l(mu_);const bool changed=!latest_cost_||sum!=cost_sum_;latest_cost_=m;if(changed){cost_sum_=sum;++cost_revision_;}RCLCPP_INFO(get_logger(),"GENERATOR_COSTMAP_RECEIPT topic=%s receipt_steady_ns=%ld header=%d.%09u checksum=%lu changed=%s content_revision=%lu dimensions=%ux%u origin=(%.3f,%.3f) resolution=%.3f",global_costmap_topic_.c_str(),std::chrono::duration_cast<std::chrono::nanoseconds>(receipt).count(),m->header.stamp.sec,m->header.stamp.nanosec,sum,changed?"true":"false",cost_revision_,m->info.width,m->info.height,m->info.origin.position.x,m->info.origin.position.y,m->info.resolution);}
 ...
 state_=State::EXTRACTING;auto started=now();cycle_map_=map;cycle_cost_=cost;cycle_revision_=rev;rx_=rx;ry_=ry;works_.clear();query_index_=0;queries_=0;
  ...
  for(auto&r:regions){
   ...
   if(!safe){RCLCPP_WARN(get_logger(),"CANDIDATE_REJECT revision=%lu id=%lu reason=NO_SAFE_APPROACH centroid=(%.3f,%.3f) bounds=(%.3f,%.3f,%.3f,%.3f)",rev,id,r.centroid.first,r.centroid.second,frontier_world_bounds(r,gm)[0],frontier_world_bounds(r,gm)[1],frontier_world_bounds(r,gm)[2],frontier_world_bounds(r,gm)[3]);suppress(id,rev,true);continue;}auto w=gc.mapToWorld(safe->first,safe->second);auto evidence=clearance_evidence(gc,w.first,w.second,approach_clearance_m_,costmap_blocked_threshold_);std::ostringstream cells;for(size_t i=0;i<evidence.examined_cells.size();++i){if(i)cells<<',';cells<<evidence.examined_cells[i];}RCLCPP_INFO(get_logger(),"GENERATOR_APPROACH_CLEARANCE revision=%lu id=%lu pose=(%.3f,%.3f) cell=(%d,%d) costmap_header=%d.%09u target=%.3f threshold=%d nearest=(%d,%d,%d,%.3f) accepted=%s cells=%s",rev,id,w.first,w.second,evidence.column,evidence.row,cost->header.stamp.sec,cost->header.stamp.nanosec,approach_clearance_m_,costmap_blocked_threshold_,evidence.nearest_column,evidence.nearest_row,evidence.nearest_cost,evidence.nearest_distance_m,evidence.accepted?"true":"false",cells.str().c_str());Work x;x.region=r;x.id=id;x.frontier_length=len;x.euclid=std::hypot(w.first-rx,w.second-ry);x.bounds=frontier_world_bounds(r,gm);x.heading=std::abs(std::atan2(std::sin(std::atan2(r.centroid.second-w.second,r.centroid.first-w.first)-yaw),std::cos(std::atan2(r.centroid.second-w.second,r.centroid.first-w.first)-yaw)));x.pose.header.frame_id=global_frame_;x.pose.header.stamp=now();x.pose.pose.position.x=w.first;x.pose.pose.position.y=w.second;double gyaw=std::atan2(r.centroid.second-w.second,r.centroid.first-w.first);x.pose.pose.orientation.z=std::sin(gyaw/2);x.pose.pose.orientation.w=std::cos(gyaw/2);const auto visible=frontier_exploration_ros2::compute_visible_reveal_gain(x.pose.pose,gm,gc,std::nullopt,visible_gain_range_m_,visible_gain_fov_deg_,visible_gain_ray_step_deg_,r.visible_reveal_bounds);x.gain=visible?visible->visible_reveal_length_m:0.;works_.push_back(std::move(x));}
